...
public class FilenameUtilsTest {
	...
	public void testName() throws Exception {
		 String basePath = "E:\\test\\ioTest\\file\\source\\";
...
//				  System.out.println("22、判断目录下是否包含指定文件或目录:"+FilenameUtils.directoryContains(basePath, fullFileName));
				  ...
				  Collection extensions = new ArrayList<Object>();
				  extensions.add("txt");
				  extensions.add("java");
	...
	public void test2() throws Exception {
		String filePathName = "E:\\test\\ioTest\\file\\source\\测试文件.txt";
		System.out.println("纯文件名："+ FilenameUtils.getBaseName(filePathName));//测试文件
		System.out.println("后缀名："+ FilenameUtils.getExtension(filePathName));//txt
		System.out.println("文件名带后缀名："+ FilenameUtils.getName(filePathName));//测试文件.txt
		System.out.println("路径："+ FilenameUtils.getPath(filePathName));//test\ioTest\file\source\
		System.out.println("全路径："+ FilenameUtils.getFullPath(filePathName));//E:\test\ioTest\file\source\
		System.out.println("全路径至最后一层文件夹："+ FilenameUtils.getFullPathNoEndSeparator(filePathName));//E:\test\ioTest\file\source
		System.out.println("路径至最后一层文件夹："+ FilenameUtils.getPathNoEndSeparator(filePathName));//test\ioTest\file\source
		System.out.println("前缀绝对地址："+ FilenameUtils.getPrefix(filePathName));//E:\
		System.out.println("前缀长度："+ FilenameUtils.getPrefixLength(filePathName));//3
